...
if (isset($_POST['textsearch2'])) {
  $colname_Rec_debtor = $_POST['textsearch2'];
} else {
  $colname_Rec_debtor ="";
}

$query1 = sprintf("SELECT paydebt.payDate,debtor.debtorID, debtor.FirstName, debtor.LastName,SUM(receipt.receiptOwe) AS 'total_owe',SUM(paydebt.amount) AS 'total_pay',SUM(receipt.receiptLeft) AS 'total_left'
FROM paydebt, debtor, receipt 
WHERE paydebt.receiptID = receipt.receiptID AND receipt.debtorID = debtor.debtorID AND (debtor.FirstName LIKE %s OR debtor.LastName LIKE %s) 
GROUP BY debtor.FirstName,debtor.LastName
ORDER BY debtor.FirstName, debtor.LastName DESC",
...
<title>ค้นหาจากชื่อ : ประวัติการชำระหนี้</title>
...
<p>&nbsp;</p>
<center>
<h1>ผลการค้นหาประวัติการชำระหนี้<br>จากชื่อ-นามสกุล</td></h1>
<font class="text">พบจำนวน&nbsp;<b><?php echo $totalRows_Rec_debtor ?> &nbsp;</b>รายการ</font>
<p>&nbsp;</p>
<table class="table_detail">
        <tr class="tr_head2">
        <td width="200">รหัสลูกหนี้</td>
        ...
        <?php do { ?>
    <tr class="tr_center">
        <td><?php echo $row_Rec_debtor1['debtorID']; ?></td>
        <td><?php echo $row_Rec_debtor1['FirstName']; ?></td>
        <td><?php echo $row_Rec_debtor1['LastName']; ?></td>
        <td><?php echo $row_Rec_debtor1['total_owe']; ?></td>
        <td><?php echo $row_Rec_debtor1['total_pay']; ?></td>
        <td><?php echo $row_Rec_debtor1['total_left']; ?></td>
        <td><a href="payment_history.php?cid=<?php echo $row_Rec_debtor1['debtorID']; ?>"><img src="img/detail.png"</a></td>
    </tr>
    <?php } while ($row_Rec_debtor1 = mysql_fetch_assoc($Rec_debtor1)); ?>
    </table></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
